...
**起動時**および**復帰時**に `ipv6.exe` が自動的に実行されるように、Windows タスク スケジューラを構成します：

1. `Win + X` を押して、**Windows PowerShell (管理者)** を開きます。
2. プロジェクトディレクトリに移動し、タスクをインポートします（コードが `D:\ipv6` にあると仮定します。XML のパスを適宜置き換えてください）：
```powershell
schtasks /create /tn "IPv6_Monitor" /xml .\task.xml /f
```
3. インポートが成功すると、システムが起動またはスリープ/休止状態から復帰するたびに、スクリプトが自動的にトリガーされてメールが送信されます。
